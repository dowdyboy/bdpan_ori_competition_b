...
class OriModelV4(nn.Layer):

    def __init__(self, num_classes=4, is_pretrain=True):
        super(OriModelV4, self).__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Linear(all_feats_out_channel[-1] * 12, all_feats_out_channel[-1] * 3),
            nn.Hardswish(),
            nn.Dropout(p=0.15),
            nn.Linear(all_feats_out_channel[-1] * 3, num_classes))
